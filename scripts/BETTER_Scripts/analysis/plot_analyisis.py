...
parser = argparse.ArgumentParser(description='Plot analysis results for BETTER dataset')
...
greedy_results = {}
greedy_path = f"results/BETTER/zeroshot/{split}/en_string_scores.csv"
...
with open(greedy_path, newline='') as csvfile:
        ...
        for row in reader:
                ...
                if model in models:
                        greedy_results[model] = float(row['better'])

voter_f1_results = {}
voter_f1_path = f"results/BETTER/zeroshot/{split}/en_string/voterf1_scores.csv"
with open(voter_f1_path, newline='') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
                model = row['file'].replace("_1.jsonl", "")
                if model in models:
                        voter_f1_results[model] = float(row['better'])


csv_path = f"results/BETTER/zeroshot/{split}/en_string_analysis.csv"
model_stats = {}
...
with open(csv_path, newline='') as csvfile:
    ...
    for row in reader:
        ...
        #TODO: Hau kendu geo trasnfentzia kalkulatzeko
        if "MUCR1" in model:
                continue
...
#ax.set_title('Model Performance Analysis on BETTER Dataset', fontsize=15, fontweight='bold', pad=20)
ax.set_xticks(x)
ax.set_xticklabels(models, rotation=20, ha='right', fontsize=11)

# Smaller axis range
y_min = min(means + voters + greedys) - 0.02
y_max = max(means + voters + greedys) + 0.02
...
plt.tight_layout()
plt.savefig(f"irudiak/BETTER_analisis_{split}.pdf", bbox_inches='tight')
plt.close()