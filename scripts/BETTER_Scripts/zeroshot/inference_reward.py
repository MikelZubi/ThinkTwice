...
prompt_class = prompt_factory(model_dir, language_name, "BETTER", think=False)
...
with open(predict_dir, 'r') as file:
    for line in file:
        ...
        pre_dict = data
        templates = [template for template in pre_dict["templates"] if template != ['ERROR'] and template != [['ERROR']]]
        templates_list = []
