...
pred_paths = "predictions/DEV"
for directory in os.listdir(pred_paths):
    pred_path_model = os.path.join(pred_paths, directory)
    for subdirectory in os.listdir(pred_path_model):
        pred_path_language = os.path.join(pred_path_model, subdirectory)
        if os.path.exists(f"predictions/DEV/{directory}/{subdirectory}/results.csv"):
            os.remove(f"predictions/DEV/{directory}/{subdirectory}/results.csv")
        for file in os.listdir(pred_path_language):
            pred_path = os.path.join(pred_path_language, file)
            pred = score(pred_path, gold_path, DatasetKind.MUC, file_type=PredictionFileType.GTT)
            # Create CSV data for this prediction
            csv_data = {
            ...
            csv_file = f"predictions/DEV/{directory}/{subdirectory}/results.csv"
