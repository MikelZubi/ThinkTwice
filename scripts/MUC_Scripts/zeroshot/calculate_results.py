...
def calculate_scores_for_directory(read,split,scorer,certainly,voterf1, maxlen, reward, language):
    gold_path = "multimuc/data/multimuc_v1.0/corrected/"+language+"/"+split+".jsonl"#IDATZI
    ground_truths = []
    ids = []
    documents = []
    labels = []
        ...
        for pred_file in prediction_files:
            ...
            with open(pred_file, 'r') as f:
                for line in f:
                    ...
                    if split == "train":
                        splited_ids = data["docid"].split("-")
                        id = splited_ids[1] + "-" + splited_ids[0] + "-" + splited_ids[2]
                    ...
                    if template == ["ERROR"] or template == [["ERROR"]] or "ERROR" in template:
                        count += 1
                        template = []
                    predictions[pred_id] = {"pred_templates":template,"gold_templates":[]}
            # Calculate scores
            results = score(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--language', type=str, default='en', help='language to use for scoring')

    args = parser.parse_args()
    calculate_scores_for_directory(args.read, args.split, args.voter, args.certainly, args.voterf1, args.maxlen, args.reward, args.language)
