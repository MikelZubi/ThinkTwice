...
def generate_prompt(data,tokenizer, language_code,step, k=2, random=True):
    ...
    with open("multimuc/data/multimuc_v1.0/corrected/"+language_code+"/train_simplified_CoT.jsonl") as f:
        count = 0
        if step == 1: #Llama 70B using deepseek reasoning as fewshot
        ...
        elif step == 2: #Llama 70B to create the template
...
if random:
    folder_path = "predictions/predictions_MUC_simplified_steps_deepseekV2/random-few/"+str(language)

else:
    folder_path = "predictions/predictions_MUC_simplified_steps_deepseekV2/first-few/"+str(language)
