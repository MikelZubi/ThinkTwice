        ...
        if k > 0:
            for line in all_train:
                ...
                if step == 1:
                    ...
                    prompt.append({"role":"assistant","content":inputs["reasoning"]})
                elif step == 2:
                    template_str = json.dumps(inputs["templates"], ensure_ascii=False)
                    prompt.append({"role":"user","content":inputs["reasoning"]})
...
full_out_step_1 = tokenizer.decode(result_1[0].prompt_token_ids) + result_1[0].outputs[0].text
pred_dict[docids[0]]["example_prompt_1"] = full_out_step_1
...
full_out_step_2= tokenizer.decode(result_2[0].prompt_token_ids) + result_2[0].outputs[0].text
pred_dict[docids[0]]["example_prompt_2"] = full_out_step_2
...
if random:
    folder_path = "predictions/predictions_MUC_simplified_steps_70B/random-few/"+str(language)

else:
    folder_path = "predictions/predictions_MUC_simplified_steps_70B/first-few/"+str(language)
