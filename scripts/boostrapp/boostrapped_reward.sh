...
for MODELNAME in "${MODELS_MUC[@]}"; do
    ...
    REWARD_FILE="results/MUC/zeroshot/${split}/${language}/Reward/${MODELNAME}-Reward_1.jsonl"
    ...
    python scripts/boostrapp/boostrapped_reward.py --reward_file ${REWARD_FILE} --boostrapp_file ${READ_FILE} --output_file ${OUT_DIR}
done
python scripts/MUC_Scripts/zeroshot/calculate_results.py --read results/MUC/zeroshot/$split/$language/boostrapp${BOOSTRAP_N}/Reward --split $split --language $language --reward
