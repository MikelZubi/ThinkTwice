...
def read_gold_templates_list(
    data: List, convert_doc_id: bool = True, sanitize_special_chars: bool = False
) -> OrderedDict:
    def _sanitize_special_char(raw_str: str) -> str:
        return (
            raw_str.replace("'", "").replace('"', "").replace("\\", "").replace("-", "")
...
def load_pred_dict(pred_data: Dict) -> OrderedDict:
    preds = OrderedDict()
    for docid in pred_data:
        pred_templates = pred_data[docid]["pred_templates"]
        for t in pred_templates:
            if t["incident_type"] == "ki":
                t["incident_type"] = "kidnapping"
        preds[docid] = pred_data[docid]["pred_templates"]
